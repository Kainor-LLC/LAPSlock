...
- ✅ **Shipping icon** — keycap with a keyhole, navy field, generated by
  `design/icons/render-icon.py`. Replaced the previous mark during the 2026-08-27 rename.
...
- ⬜ **Alternate icon picker (orange field)** — the orange asset is rendered and committed,
  ...
  changed the icon for LAPSlock"), so it is two taps for a preference set once; and icon
...
## ⚠️ Three things to resolve before building the meter
...
Backend infrastructure is provisioned (see "Backend (Azure)"). IAP and App Attest need
Apple; the Function does not.

11. `/entitlement` Function: API contract, ES256 key, licences table, code
12. Stripe
13. IAP products, StoreKit trial, and free-tier gating
...
Two workstreams can run in parallel now, and they do not block each other:

- **Backend:** the `/entitlement` API contract, then the ES256 key, licences table, and
  Function code. The contract is the piece every later billing and gating decision hangs
  off, so it is worth doing carefully and publishing.
- **Client:** `LicensingKit` and the reveal meter. Self-contained, needs no backend, and it
  is the half of the revenue path that can ship first.

Before either, the cheap high-value items: verify Keychain survival across app deletion on
device (the meter's whole anti-reset property rests on it), and file the Microsoft support
case for the macOS 500 so the write-up has an ending.
