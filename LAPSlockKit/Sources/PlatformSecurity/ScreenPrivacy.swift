...
///
/// Known cosmetic bug: after tapping reveal, the "Hidden" cover shows for about two
/// seconds before the credential appears. The Face ID prompt takes the scene to .inactive,
/// the secret is published while the scene is still .inactive, and the cover clears only
/// when scenePhase catches up. Two fixes that tracked the transition more precisely both
/// failed in the dangerous direction on device: the credential appeared in the app
/// switcher. The condition below errs toward hiding and stays.
///
/// The correct fix is not in this file: hold the fetched secret in DeviceDetailModel and
/// publish it only once scenePhase is .active. Whatever is attempted, verify on device in
/// this order and treat the second as blocking: (1) reveal shows no flash, (2) with a
/// credential up, swipe to the app switcher; the card must show "Hidden".
                ...
                if shouldCover {
                    ZStack {
                        ...
                        VStack(spacing: 12) {
                            ...
                            Text("Hidden")
                                .font(.headline)
                            Text("LAPSlock hides credentials when it isn't in the foreground.")
                                .font(.footnote)
